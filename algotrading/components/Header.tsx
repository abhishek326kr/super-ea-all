...
export default function Header() {
  ...
  return (
    <>
      <header style={headerStyle}>
        ...
        >
          ...
          <button
            ...
            className="lg:hidden relative text-white p-2 hover:bg-white/10 rounded-full transition-colors"
            style={{ zIndex: 10001 }}
      ...
      {/* Mobile Menu Overlay - OUTSIDE the header to avoid stacking context issues */}
      <div
        className={`fixed inset-0 bg-black/60 backdrop-blur-sm transition-opacity duration-300 lg:hidden ${isOpen ? "opacity-100 visible" : "opacity-0 invisible pointer-events-none"
          }`}
        style={{ zIndex: 9999 }}
        onClick={() => setIsOpen(false)}
      />

      {/* Mobile Menu Drawer - OUTSIDE the header to avoid stacking context issues */}
      <div
        className={`fixed top-0 right-0 h-full w-[85vw] max-w-sm bg-[#0a0f19] border-l border-white/10 transform transition-transform duration-300 ease-out lg:hidden flex flex-col pt-24 px-6 shadow-2xl overflow-y-auto ${isOpen ? "translate-x-0" : "translate-x-full"
          }`}
        style={{ zIndex: 10000 }}
      >
